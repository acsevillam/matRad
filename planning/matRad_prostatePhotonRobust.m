...
% Body
cst{1,5}.Priority = 2; % overlap priority for optimization - a lower number corresponds to a higher priority
cst{1,6}{1} = struct(DoseObjectives.matRad_SquaredOverdosing(300,10));
...
cst{ixCTV,6}{1} = struct(DoseObjectives.matRad_SquaredDeviation(300,30));
...
pln.multScen.numOfShiftScen = [100 100 100];
...
matRad_exportBeamPositions(pln_interval,stf)
...
[dij_interval, pln_interval]=matRad_calcDoseInterval(dij,pln,20,80);
...
% Max interval dose
tmpResultGUI2 = matRad_calcCubes(wOnes,dij_interval,2);
