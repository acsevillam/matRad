function [robCube,robPassRate,robustnessFig] = robustnessIndex1(meanCube,stdCube,refDose,criteria,slice,ct,cst)
...
if exist('slice','var') && ~isempty(slice)
    ...
    matRad_plotSliceWrapper(gca,ct,cst,refScen,robCube.*targetMask,plane,slice,[],[],[],myColormap,doseWindow,[],[],'Deviation-Uncertainty',[],'LineWidth',1.5);
    ...
    b.Callback    = @(es,ed)  matRad_plotSliceWrapper(ax1,ct,cst,refScen,robCube.*targetMask,plane,round(es.Value),[],[],[],myColormap,doseWindow,[],[],'Deviation-Uncertainty',[],'LineWidth',1.5);  
    
    subplot(1,2,2);
    numSlices = ct.cubeDim(3);
    matRad_plotSliceWrapper(gca,ct,cst,refScen,(robCube<=1).*doseMask,plane,slice,[],[],[],[],[0 2.01],[],[],[],[],'LineWidth',1.5);
    title({[num2str(robPassRate,5) '% of points ' ...
        'pass robustness criterion (' num2str(meanDoseThreshold) '% / ' ...
        num2str(stdThreshold) '%)']});
    ...
    b = uicontrol('Parent',f2,'Style','slider','Position',[430,5,280,23],...
        'value',slice, 'min',1, 'max',numSlices,'SliderStep', [1/(numSlices-1) , 1/(numSlices-1)]);
    b.Callback    = @(es,ed)  matRad_plotSliceWrapper(ax2,ct,cst,refScen,(robCube<=1).*doseMask,plane,round(es.Value),[],[],[],[],[0 2.01],[],[],[],[],'LineWidth',1.5);  
