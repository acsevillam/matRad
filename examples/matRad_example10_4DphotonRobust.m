...
[resultGUIrobust4D, ~] = matRad_calc4dDose(ct, pln, dij, stf, cst, resultGUIrobust,totalPhaseMatrix);
...
maxDose       = max([max(resultGUI.(quantityOpt)(:,:,slice)) ...
                     max(resultGUIrobust.(quantityOpt)(:,:,slice)) ...
                     max(resultGUI4D.(quantityOpt)(:,:,slice)) ...
                     max(resultGUI4D.('accPhysicalDose')(:,:,slice)) ...
                     max(resultGUIrobust4D.(quantityOpt)(:,:,slice)) ...
                     max(resultGUIrobust4D.('accPhysicalDose')(:,:,slice))])+1e-4;
...
figure
subplot(121),matRad_plotSliceWrapper(gca,ct,cst,1,resultGUIrobust4D.(quantityOpt),plane,slice,[],[],colorcube,[],[0 maxDose],doseIsoLevels);title('robust plan')
subplot(122),matRad_plotSliceWrapper(gca,ct,cst,1,resultGUIrobust4D.('accPhysicalDose'),plane,slice,[],[],colorcube,[],[0 maxDose],doseIsoLevels);title('robust plan dose accumulation')
...
matRad_plotSliceWrapper(gca,ct,cst,1,resultGUIrobust.([quantityOpt '_scen' num2str(round(numScen))]),plane,slice,[],[],colorcube,[],doseWindow,doseIsoLevels);
...
if strcmp(env,'MATLAB') || str2double(envver(1)) >= 5
    ...
    set(b,'Callback',@(es,ed)  matRad_plotSliceWrapper(gca,ct,cst,round(get(es,'Value')),resultGUIrobust.([quantityOpt '_scen' num2str(round(get(es,'Value')))]),plane,slice,[],[],colorcube,[],doseWindow,doseIsoLevels));
end
...
[caSampRob, mSampDoseRob, plnSampRob, resultGUInomScen] = matRad_sampling(ct,stf,cst,pln,resultGUIrobust.w,structSel,samplingMultScen,dvhSamplingArgs{:});
[cstStatRob, resultGUISampRob, metaRob, ~, robustnessFigRob1] = ...
...
figure(robustnessFigRob1),title('Robustness index 1 - robust');
