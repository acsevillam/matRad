function runPhotonBreastInterval2Workflow(varargin)
% runPhotonBreastInterval2Workflow Run the photon breast INTERVAL2 workflow.
% Usage: runPhotonBreastInterval2Workflow('caseID','4136','rootPath',userDataRoot,'cacheRootPath',fullfile(userDataRoot,'output','cache'),'randomSeed',[])
%
% This macro is breast-specific and uses the breast interval2 template.
...
if exist('ensurePlanWorkflowAvailable','file') == 2
    ensurePlanWorkflowAvailable('runPhotonBreastInterval2Workflow');
end

userDataRoot = fileparts(macroRoot);

macroDefaults = struct();
macroDefaults.caseID = '4136';
...
workflowConfig.precompute.robustPlans.robust_1.scenario.shiftSD = [4 8 6];
