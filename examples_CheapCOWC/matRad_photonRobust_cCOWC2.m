...
if run_config.robustness == "c-COWC"
    output_folder = ['output' filesep run_config.description filesep run_config.robustness filesep run_config.plan_beams filesep run_config.plan_target filesep run_config.plan_objectives filesep run_config.scen_mode filesep num2str(run_config.beta1) '_to_' num2str(run_config.beta2) filesep datestr(datetime,'yyyy-mm-dd HH-MM-SS')];
else
    output_folder = ['output' filesep run_config.description filesep run_config.robustness filesep run_config.plan_beams filesep run_config.plan_target filesep run_config.plan_objectives filesep run_config.scen_mode filesep datestr(datetime,'yyyy-mm-dd HH-MM-SS')];
end
...
switch run_config.robustness
    case 'c-COWC'
        cst_robust{ixTarget,8}{1}.beta1 = run_config.beta1;
        cst_robust{ixTarget,8}{1}.p1 = run_config.p1;
        cst_robust{ixTarget,8}{1}.beta2 = run_config.beta2;
        cst_robust{ixTarget,8}{1}.p2 = run_config.p2;
        for i=1:length(cst_robust{ixTarget,6})
            cst_robust{ixTarget,6}{i}.robustness  = run_config.robustness;
        end
    otherwise
        for i=1:length(cst_robust{ixTarget,6})
            cst_robust{ixTarget,6}{i}.robustness  = run_config.robustness;
...
f_robust = matRad_calcObjectiveFunction(w_robust,dij_robust,cst,pln_robust);
...
%% Obtain dose statistics
[dvh_robust,dqi_robust] = matRad_indicatorWrapper(cst_robust,pln_robust,resultGUI_robust.physicalDose*pln_robust.numOfFractions,[],[],run_config.doseWindow_dvh);
...
%% Perform sampling
[caSamp, mSampDose, plnSamp, resultGUInomScen] = matRad_sampling(ct,stf_robust,cst_robust,pln_robust,resultGUI_robust.w,structSel,multScen);

%% Perform sampling analysis
varargin.GammaCriterion = run_config.GammaCriterion; % [%  mm]
[cstStat, resultGUISamp, meta] = matRad_samplingAnalysis(ct,cst_robust,plnSamp,caSamp, mSampDose, resultGUInomScen, varargin);

%% Multi-scenario dose volume histogram (DVH)
figure,set(gcf,'Color',[1 1 1],'position',[10,10,600,400]);
matRad_showDVH_sampledScen(caSamp,dvh_robust,cst_robust,plnSamp,[1:multScen.totNumShiftScen],run_config.doseWindow_dvh);
...
numSlices = ct.cubeDim(3);
slice = round(pln_robust.propStf.isoCenter(1,3)./ct.resolution.z);
matRad_plotSliceWrapper(gca,ct,cst_robust,1,resultGUISamp.(quantityMap)*pln_robust.numOfFractions,plane,slice,[],[],colorcube,[],doseWindow,doseIsoLevels,[],'Dose uncertainty [Gy]');
b = uicontrol('Parent',f,'Style','slider','Position',[50,5,420,23],...
      'value',slice, 'min',1, 'max',numSlices,'SliderStep', [1/(numSlices-1) , 1/(numSlices-1)]);
b.Callback = @(es,ed)  matRad_plotSliceWrapper(gca,ct,cst_robust,1,resultGUISamp.(quantityMap)*pln_robust.numOfFractions,plane,round(es.Value),[],[],colorcube,[],doseWindow,doseIsoLevels,[],'Dose uncertainty [Gy]');
...
numSlices = ct.cubeDim(3);
slice = round(pln_robust.propStf.isoCenter(1,3)./ct.resolution.z);
matRad_plotSliceWrapper(gca,ct,cst_robust,1,resultGUISamp.(analysis).(quantityMap).*target_mask,plane,slice,[],[],colorcube,[],doseWindow,[],[],'Relative uncertainty');
b = uicontrol('Parent',f,'Style','slider','Position',[50,5,420,23],...
      'value',slice, 'min',1, 'max',numSlices,'SliderStep', [1/(numSlices-1) , 1/(numSlices-1)]);
b.Callback = @(es,ed)  matRad_plotSliceWrapper(gca,ct,cst_robust,1,resultGUISamp.(analysis).(quantityMap).*target_mask,plane,round(es.Value),[],[],colorcube,[],doseWindow,[],[],'Relative uncertainty');
...
results.structures{ixCTV,1}=cst_robust{ixCTV,2};
...
UncertaintyIndex2=nnz(resultGUISamp.uncertaintyAnalysis.robustnessViolationCube(cst_robust{ixCTV,4}{1,1}))/numel(cst_robust{ixCTV,4}{1,1});
...
h2=histogram(resultGUISamp.uncertaintyAnalysis.relativeUncertaintyCube(cst_robust{ixCTV,4}{1,1}),edges,'Normalization','probability', 'DisplayStyle','stairs','LineWidth', 2);
xlabel('Relative uncertainty');
ylabel('[%]');
ylim([0 0.5]);
...
%% print results
[results] = matRad_printResults(run_config,results,cst_robust,pln,dqi_sampled);
