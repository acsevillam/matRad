    ...
    case 'prostate'
        switch plan_objectives
            case '1'
                ...
                cst{2,6}{1} = struct(DoseObjectives.matRad_MaxDVH(300,60,35));
                ...
                cst{3,6}{1} = struct(DoseObjectives.matRad_MaxDVH(300,40,40));
                ...
                if(target=="PTV")
                    ...
                    cst{ixTarget,6}{2} = struct(DoseObjectives.matRad_MinDVH(400,78,95));
                    cst{ixTarget,6}{2}.robustness  = 'none';
                    cst{ixTarget,6}{3} = struct(DoseObjectives.matRad_MaxDVH(400,81,5));
                    cst{ixTarget,6}{3}.robustness  = 'none';
                    cst{ixTarget,6}{4} = struct(DoseObjectives.matRad_MaxDVH(400,83.5,0));
                    cst{ixTarget,6}{4}.robustness  = 'none'; 
                    %cst{ixTarget,6}{5} = struct(DoseConstraints.matRad_MinMaxDVH(p,95,100));
                else
                    % CTV
                    p=78;
                    ixCTV = 7;
                    ...
                    cst{ixTarget,6}{1} = struct(DoseObjectives.matRad_SquaredDeviation(800,p));
                    cst{ixTarget,6}{1}.robustness  = 'none';
                    cst{ixTarget,6}{2} = struct(DoseObjectives.matRad_MinDVH(800,78,99));
                    cst{ixTarget,6}{2}.robustness  = 'none';
                    cst{ixTarget,6}{3} = struct(DoseObjectives.matRad_MaxDVH(800,81,5));
                    cst{ixTarget,6}{3}.robustness  = 'none';
                    cst{ixTarget,6}{4} = struct(DoseObjectives.matRad_MaxDVH(800,83.5,0));
                    cst{ixTarget,6}{4}.robustness  = 'none'; 
                    %cst{ixTarget,6}{5} = struct(DoseConstraints.matRad_MinMaxDVH(p,95,100));
                end
            ...
            case '2'
                ...
                % Bladder
                cst{2,5}.Priority = 3; % overlap priority for optimization - a lower number corresponds to a higher priority
                cst{2,6}{1} = struct(DoseObjectives.matRad_MaxDVH(300,60,45));
                cst{2,6}{1}.robustness  = 'none';

                % Rectum
                cst{3,5}.Priority = 3; % overlap priority for optimization - a lower number corresponds to a higher priority
                cst{3,6}{1} = struct(DoseObjectives.matRad_MaxDVH(300,50,55));
                cst{3,6}{1}.robustness  = 'none';
