...
switch patient
    case 'prostate'
        ...
        if(target=="PTV")
            ...
            % PTV
            ixTarget = 8;
            if exist('cst{ixTarget,3}','var') && ~isempty(cst{ixTarget,3})
        ...
        switch plan_objectives
            ...
            case '2'
                ...
                % Rectum
                cst{3,5}.Priority = 3; % overlap priority for optimization - a lower number corresponds to a higher priority
                cst{3,6}{1} = struct(DoseObjectives.matRad_MaxDVH(600,50,50));
                cst{3,6}{1}.robustness  = 'none';
            ...
            case '3'
                ...
                % Bladder
                cst{2,5}.Priority = 3; % overlap priority for optimization - a lower number corresponds to a higher priority
                cst{2,6}{1} = struct(DoseObjectives.matRad_MaxDVH(600,60,55));
                cst{2,6}{1}.robustness  = 'none';

                % Rectum
                cst{3,5}.Priority = 3; % overlap priority for optimization - a lower number corresponds to a higher priority
                cst{3,6}{1} = struct(DoseObjectives.matRad_MaxDVH(600,50,60));
                cst{3,6}{1}.robustness  = 'none';
            ...
            case '4'
                ...
                % Bladder
                cst{2,5}.Priority = 3; % overlap priority for optimization - a lower number corresponds to a higher priority
                cst{2,6}{1} = struct(DoseObjectives.matRad_MaxDVH(600,60,65));
                cst{2,6}{1}.robustness  = 'none';

                % Rectum
                cst{3,5}.Priority = 3; % overlap priority for optimization - a lower number corresponds to a higher priority
                cst{3,6}{1} = struct(DoseObjectives.matRad_MaxDVH(600,50,70));
                cst{3,6}{1}.robustness  = 'none';
            ...
            case '5'
                ...
                % Bladder
                cst{2,5}.Priority = 3; % overlap priority for optimization - a lower number corresponds to a higher priority
                cst{2,6}{1} = struct(DoseObjectives.matRad_MaxDVH(600,60,75));
                cst{2,6}{1}.robustness  = 'none';

                % Rectum
                cst{3,5}.Priority = 3; % overlap priority for optimization - a lower number corresponds to a higher priority
                cst{3,6}{1} = struct(DoseObjectives.matRad_MaxDVH(600,50,80));
                cst{3,6}{1}.robustness  = 'none';
    ...
    case 'breast'
        ...
        switch plan_objectives
            ...
            case '3'
                ...
                % Ipsilateral Lung
                cst{3,5}.Priority = 2; % overlap priority for optimization - a lower number corresponds to a higher priority
                cst{3,6}{1} = struct(DoseObjectives.matRad_MaxDVH(400,20,50));
                cst{3,6}{1}.robustness  = 'none';
                %cst{3,6}{2} = struct(DoseConstraints.matRad_MinMaxDVH(20,0,20));

                % Heart
                cst{4,5}.Priority = 2; % overlap priority for optimization - a lower number corresponds to a higher priority
                cst{4,6}{1} = struct(DoseObjectives.matRad_MeanDose(250,9));
                cst{4,6}{1}.robustness  = 'none';
            ...
            case '4'
                ...
                % Ipsilateral Lung
                cst{3,5}.Priority = 2; % overlap priority for optimization - a lower number corresponds to a higher priority
                cst{3,6}{1} = struct(DoseObjectives.matRad_MaxDVH(400,20,65));
                cst{3,6}{1}.robustness  = 'none';
                %cst{3,6}{2} = struct(DoseConstraints.matRad_MinMaxDVH(20,0,20));

                % Heart
                cst{4,5}.Priority = 2; % overlap priority for optimization - a lower number corresponds to a higher priority
                cst{4,6}{1} = struct(DoseObjectives.matRad_MeanDose(250,11));
                cst{4,6}{1}.robustness  = 'none';
            ...
            case '5'
                ...
                % Ipsilateral Lung
                cst{3,5}.Priority = 2; % overlap priority for optimization - a lower number corresponds to a higher priority
                cst{3,6}{1} = struct(DoseObjectives.matRad_MaxDVH(400,20,80));
                cst{3,6}{1}.robustness  = 'none';
                %cst{3,6}{2} = struct(DoseConstraints.matRad_MinMaxDVH(20,0,20));

                % Heart
                cst{4,5}.Priority = 2; % overlap priority for optimization - a lower number corresponds to a higher priority
                cst{4,6}{1} = struct(DoseObjectives.matRad_MeanDose(250,13));
                cst{4,6}{1}.robustness  = 'none';
