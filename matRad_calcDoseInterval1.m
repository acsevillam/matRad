function [dij_dummy, pln_dummy,dij_interval] = matRad_calcDoseInterval1(ct,cst,stf,pln,dij,structSel)
...
dij_interval.radius=cell(dij.doseGrid.numOfVoxels,1);
...
for it=1:numel(subIx)
    ...
    dij_interval.radius{subIx(it)}=sparse(dij_tmp'*diag(pln.multScen.scenProb)*dij_tmp);
