...
function test_samplingForwardDoseWorkerEstimateUsesConservativeFloor
samplingContext = struct();
samplingContext.ct = struct('cubeDim', [20 20 10]);
samplingContext.subIx = (1:100)';
samplingContext.stf = repmat(struct('gantryAngle', 0), 1, 2);

minWorkerBytes = 8 * 1024^3;
[workerBytes, details] = matRad_estimateSamplingForwardDoseWorkerBytes( ...
    samplingContext, 10e6, 2e6, 1e6, minWorkerBytes);

assertTrue(workerBytes >= minWorkerBytes);
assertElementsAlmostEqual(details.forwardDoseWorkspaceBytes, ...
                          minWorkerBytes, 'absolute', 1);
assertEqual(details.numBeams, 2);
assertEqual(details.selectedVoxelCount, 100);

function test_samplingParallelOverheadExposesPlannerComponents
workerDetails = struct('forwardDoseWorkspaceBytes', 16 * 1024^3);
sampleDoseBytes = 64 * 1024^2;
...
assertEqual(details.chunkBufferBytesPerTask, sampleDoseBytes);
assertEqual(details.parforResultCopyBytesPerTask, ...
            sampleDoseBytes + sampleResultBytes);
assertElementsAlmostEqual(details.parallelWorkerOverheadBytes, ...
                          0.30 * workerDetails.forwardDoseWorkspaceBytes, ...
                          'absolute', 1);
assertEqual(details.parallelOverheadBytesPerTask, overheadBytes);

function test_samplingApolo544123EstimateReducesWorkersWithoutUpperBound
giB = 1024^3;
numScenarios = 49;
rawWorkerBytes = 16.05 * giB;
usableBudgetBytes = 306 * giB;
sampleDoseBytes = 64 * 1024^2;
sampleResultBytes = 64 * 1024;
workerDetails = struct('forwardDoseWorkspaceBytes', 16 * giB);

legacyResultBytesPerTask = sampleDoseBytes + sampleResultBytes;
legacyPlan = matRad_planMemoryLimitedParallelTasks( ...
    numScenarios, rawWorkerBytes, ...
    'stageName', 'sampling forward dose', ...
    'resultBytesPerTask', legacyResultBytesPerTask, ...
    'accumulatorBytes', 64 * 1024^2, ...
    'memoryBudgetBytes', usableBudgetBytes, ...
    'safetyFactor', 1.2, ...
    'minWorkerMemoryBytes', 4 * giB, ...
    'environment', struct(), ...
    'matRadCfg', MatRad_Config.instance());

[parallelOverheadBytes, ~] = matRad_estimateSamplingParallelOverheadBytes( ...
    sampleDoseBytes, sampleResultBytes, workerDetails);
newPlan = matRad_planMemoryLimitedParallelTasks( ...
    numScenarios, rawWorkerBytes, ...
    'stageName', 'sampling forward dose', ...
    'resultBytesPerTask', legacyResultBytesPerTask + parallelOverheadBytes, ...
    'accumulatorBytes', 64 * 1024^2, ...
    'memoryBudgetBytes', usableBudgetBytes, ...
    'safetyFactor', 1.2, ...
    'minWorkerMemoryBytes', 4 * giB, ...
...
assertEqual(legacyPlan.workerUpperBound, 15);
assertTrue(newPlan.workerUpperBound < legacyPlan.workerUpperBound);
assertTrue(newPlan.workerUpperBound <= 12);
assertTrue(isempty(newPlan.configuredWorkerUpperBound));
assertElementsAlmostEqual(newPlan.safetyFactor, 1.2, 'absolute', 1e-12);

function test_memoryLimitedParallelTaskPlannerReducesSamplingChunkSize
parallelPlan = matRad_planMemoryLimitedParallelTasks( ...
    6, 100e6, ...
    'stageName', 'sampling forward dose', ...
    'resultBytesPerTask', 10e6, ...
    'accumulatorBytes', 40e6, ...
    'memoryBudgetBytes', 380e6, ...
    'safetyFactor', 1, ...
    'environment', struct(), ...
    'matRadCfg', MatRad_Config.instance());
...
function test_memoryLimitedParallelTaskPlannerFallsBackForSamplingMemory
parallelPlan = matRad_planMemoryLimitedParallelTasks( ...
    6, 200e6, ...
    'stageName', 'sampling forward dose', ...
    'resultBytesPerTask', 20e6, ...
    'memoryBudgetBytes', 300e6, ...
    'safetyFactor', 1, ...
    'environment', struct(), ...
    'matRadCfg', MatRad_Config.instance());
