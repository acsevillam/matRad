function matRad_showDVH_multiScen(dvh,cst,pln,scenarios,lineStyleIndicator)
...
for k = scenarios   
    for i = 1:numOfVois
        if cst{i,5}.Visible
            ...
            % behind
            ix{1,k}      = max([1 find(dvh{1,k}(i).volumePoints>0,1,'last')]);
            currDvh{1,k} = [dvh{1,k}(i).doseGrid(1:ix{1,k});dvh{1,k}(i).volumePoints(1:ix{1,k})];

            plot(currDvh{1,k}(1,:),currDvh{1,k}(2,:),'LineWidth',1,'Color',colorMx(i,:), ...
                'LineStyle',lineStyles{lineStyleIndicator},'DisplayName',cst{i,2})

            maxDVHvol  = max(maxDVHvol,max(currDvh{1,k}(2,:)));
            maxDVHdose = max(maxDVHdose,max(currDvh{1,k}(1,:)));
        end
    end
end
fontSizeValue = 14;
