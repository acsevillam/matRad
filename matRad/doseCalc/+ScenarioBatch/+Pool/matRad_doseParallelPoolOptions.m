function options = matRad_doseParallelPoolOptions(owner, matRadCfg, context)
% matRad_doseParallelPoolOptions maps resource contracts to pool options
%
% call
%   options = ScenarioBatch.Pool.matRad_doseParallelPoolOptions(owner,matRadCfg,context)
%
% input
...
%
% output
%   options:    name-value cell array for
%               ScenarioBatch.Pool.matRad_configureSafeDoseParallelPool
%
% References
%   -
%
...
if ~isstruct(parallelOptions) || ~isscalar(parallelOptions)
    matRadCfg.dispError('%s must be a scalar struct.', context);
end
...
for i = 1:numel(fields)
    ...
    switch fieldName
        ...
        case 'workerUpperBound'
            matRad_validateOptionalPositiveInteger(value, fieldName, matRadCfg);
            options = [options {'workerUpperBound', value}]; %#ok<AGROW>
        otherwise
            matRadCfg.dispError('Unknown dose parallel option "%s" in %s.', ...
                                fieldName, context);
    end
end
end

function matRad_validateOptionalPositiveInteger(value, valueName, matRadCfg)
if isempty(value)
    return
end
