...
if useParallel
    ...
    if serialEstimatedMB <= memoryLimitMB
        matRadCfg.dispWarning(['Parallel probabilistic omega estimated memory is %.2f MB, ', ...
                               'which exceeds MemoryLimitMB %.2f MB. Falling back to ', ...
                               'serial probabilistic omega for the omega pass.\n'], ...
                              estimatedMB, memoryLimitMB);
        useParallel = false;
        return
    end

    matRadCfg.dispError(['Probabilistic omega estimated memory is %.2f MB in parallel ', ...
                         'and %.2f MB in serial, which exceeds MemoryLimitMB %.2f MB. ', ...
                         'Increase cfg.MemoryLimitMB, reduce the number of bixels, ', ...
                         'scenarios, VOIs, or batch rows.'], ...
