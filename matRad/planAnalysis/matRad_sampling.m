...
inputBreakdown = struct();
inputBreakdown.ct = matRad_variableBytes(samplingContext.ct);
inputBreakdown.stf = matRad_variableBytes(samplingContext.stf);
inputBreakdown.cst = matRad_variableBytes(samplingContext.cst);
inputBreakdown.pln = matRad_variableBytes(samplingContext.pln);
inputBreakdown.w = matRad_variableBytes(samplingContext.w);
inputBreakdown.cstEval = matRad_variableBytes(samplingContext.cstEval);
inputBreakdown.subIx = matRad_variableBytes(samplingContext.subIx);
inputBreakdown.dvhPoints = matRad_variableBytes(samplingContext.dvhPoints);
inputBreakdown.refGy = matRad_variableBytes(samplingContext.refGy);
inputBreakdown.refVol = matRad_variableBytes(samplingContext.refVol);
inputBreakdown.doseMapping = matRad_variableBytes(samplingContext.doseMapping);
inputBreakdown.metadata = inputBreakdown.cstEval + inputBreakdown.subIx + ...
    inputBreakdown.dvhPoints + inputBreakdown.refGy + inputBreakdown.refVol;
inputBytes = inputBreakdown.ct + inputBreakdown.stf + inputBreakdown.cst + ...
             inputBreakdown.pln + inputBreakdown.w + ...
             inputBreakdown.metadata + inputBreakdown.doseMapping;
...
[workerBytes, workerDetails] = matRad_estimateSamplingForwardDoseWorkerBytes( ...
    ...
    samplingResourceConfig.minForwardDoseWorkerMemoryBytes);
resultStorageBytesPerTask = sampleDoseBytes + sampleResultBytes;
[parallelOverheadBytesPerTask, parallelOverheadDetails] = ...
    matRad_estimateSamplingParallelOverheadBytes( ...
    sampleDoseBytes, sampleResultBytes, workerDetails);
...
samplingMemoryEstimate.inputBreakdown = inputBreakdown;
...
samplingMemoryEstimate.resultStorageBytesPerTask = resultStorageBytesPerTask;
samplingMemoryEstimate.chunkBufferBytesPerTask = ...
    parallelOverheadDetails.chunkBufferBytesPerTask;
samplingMemoryEstimate.parallelWorkerOverheadBytes = ...
    parallelOverheadDetails.parallelWorkerOverheadBytes;
samplingMemoryEstimate.parallelOverheadBytesPerTask = ...
    parallelOverheadBytesPerTask;
samplingMemoryEstimate.parallelOverheadDetails = parallelOverheadDetails;
samplingMemoryEstimate.resultBytesPerTask = resultStorageBytesPerTask + ...
    parallelOverheadBytesPerTask;
...
msg = ['matRad: Estimated sampling memory: output ', ...
       ...
       ', result ', matRad_formatSamplingBytes(samplingMemoryEstimate.resultStorageBytesPerTask), ...
       '/scenario'];
if isfield(samplingMemoryEstimate, 'parallelOverheadBytesPerTask')
    msg = [msg, ', parallel overhead ', ...
           matRad_formatSamplingBytes(samplingMemoryEstimate.parallelOverheadBytesPerTask), ...
           '/scenario'];
end
if isfield(samplingMemoryEstimate, 'chunkBufferBytesPerTask')
    msg = [msg, ', chunk buffer ', ...
           matRad_formatSamplingBytes(samplingMemoryEstimate.chunkBufferBytesPerTask), ...
           '/scenario'];
end
if isfield(samplingMemoryEstimate, 'parallelWorkerOverheadBytes')
    msg = [msg, ', worker overhead ', ...
           matRad_formatSamplingBytes(samplingMemoryEstimate.parallelWorkerOverheadBytes), ...
           '/worker'];
end
if isfield(samplingMemoryEstimate, 'inputBreakdown')
    inputBreakdown = samplingMemoryEstimate.inputBreakdown;
    msg = [msg, ', inputs ', matRad_formatSamplingBytes(samplingMemoryEstimate.inputBytes), ...
           ' (ct ', matRad_formatSamplingBytes(inputBreakdown.ct), ...
           ', stf ', matRad_formatSamplingBytes(inputBreakdown.stf), ...
           ', cst ', matRad_formatSamplingBytes(inputBreakdown.cst), ...
           ', pln ', matRad_formatSamplingBytes(inputBreakdown.pln), ...
           ', w ', matRad_formatSamplingBytes(inputBreakdown.w), ...
           ', metadata ', matRad_formatSamplingBytes(inputBreakdown.metadata), ...
           ', mapping ', matRad_formatSamplingBytes(inputBreakdown.doseMapping), ...
           ')'];
end
if isfield(samplingMemoryEstimate, 'forwardDoseWorkerDetails')
    workerDetails = samplingMemoryEstimate.forwardDoseWorkerDetails;
    msg = [msg, ', dose cube ', matRad_formatSamplingBytes(samplingMemoryEstimate.doseResultProxyBytes), ...
           ', mapping workspace ', matRad_formatSamplingBytes(samplingMemoryEstimate.doseMappingWorkspaceBytes), ...
           ', forward workspace ', matRad_formatSamplingBytes(workerDetails.forwardDoseWorkspaceBytes)];
end
if ~isempty(parallelPlan)
    msg = [msg, ', memory-limited workers ', num2str(parallelPlan.workerUpperBound), ...
           ...
           ', safety factor ', num2str(parallelPlan.safetyFactor), ...
           ', planner task ', matRad_formatSamplingBytes(parallelPlan.resultBytesPerTask), ...
