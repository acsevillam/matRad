...
for i = 1:size(cst,1)
    ...
    cstStat(i).qiStat      = calcQiStat(cstStat(i).qi,cstStat(i).percentiles,cstStat(i).w,metric);
end

end

function dvhStat = calcDVHStat(dvh,percentiles,weights)
...
dvhStat.mean.volumePoints = wMean(dvhMat,weights);
...
dvhStat.std.volumePoints = wStd(dvhMat,weights);
...
function qiStat = calcQiStat(qi,percentiles,weights,metric)
...
for j = 1:numel(fields)
    if numel([qiStruct(:).(fields{j})]) == numel(weights)
        qiStatH(1).(fields{j}) = wMean([qiStruct(:).(fields{j})],weights);
        qiStatH(2).(fields{j}) = min([qiStruct(:).(fields{j})]);
        qiStatH(3).(fields{j}) = max([qiStruct(:).(fields{j})]);
        qiStatH(4).(fields{j}) = wStd([qiStruct(:).(fields{j})],weights);
