...
if ~isfield(ct,'dvf') || isempty(ct.dvf)
    matRad_cfg.dispError('Multi-CT sampling analysis requires pull DVFs to map sampled doses to the reference CT scenario.');
end
...
function validateDvfReferenceScenario(ct,refScen,matRad_cfg)
dvfRefScen = [];
...
if ~isempty(dvfRefScen) && dvfRefScen ~= refScen
    matRad_cfg.dispError('Pull DVFs were generated for reference CT scenario %d, but sampling uses reference CT scenario %d.', ...
        dvfRefScen,refScen);
end
end
...
function hasPull = hasPullDvf(ct)
dvfType = '';
