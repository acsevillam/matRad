...
if ~iscell(ctIn) || isempty(ctIn)
    matRad_cfg.dispError('ctIn must be a non-empty cell array of ct structs.');
end
...
if max(dx) - min(dx) > tol
    matRad_cfg.dispError('CT scenarios have different x-axis spacing. Resampling is required.');
end
if max(dy) - min(dy) > tol
    matRad_cfg.dispError('CT scenarios have different y-axis spacing. Resampling is required.');
end
if max(dz) - min(dz) > tol
    matRad_cfg.dispError('CT scenarios have different z-axis spacing. Resampling is required.');
end
...
if ~(xMinCommon < xMaxCommon && yMinCommon < yMaxCommon && zMinCommon < zMaxCommon)
    matRad_cfg.dispError('CT scenarios do not share a common 3-D overlap.');
end
...
for ctScenario = 1:numOfScenarios
    ...
    if isempty(ixCrop) || isempty(iyCrop) || isempty(izCrop)
        matRad_cfg.dispError('CT scenario %d has no voxels inside the common overlap.',ctScenario);
    end

    ctCube = ctIn{ctScenario}.cubeHU{1};
    croppedCube = ctCube(iyCrop,ixCrop,izCrop);
    croppedDim = getCubeDim(croppedCube);
    originalDim = getCubeDim(ctCube);
...
if ~(all(nx == nx(1)) && all(ny == ny(1)) && all(nz == nz(1)))
    matRad_cfg.dispError('CT scenarios still have different dimensions after overlap cropping.');
end

commonGrid = struct('x',ctOut{1}.x,'y',ctOut{1}.y,'z',ctOut{1}.z);
end

function cubeDim = getCubeDim(cube)
cubeDim = size(cube);
if numel(cubeDim) < 3
    cubeDim(3) = 1;
end
