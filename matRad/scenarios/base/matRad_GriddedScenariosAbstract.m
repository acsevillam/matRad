classdef (Abstract) matRad_GriddedScenariosAbstract < matRad_ScenarioModel
    ...
    methods
        ...
        %% set methods
        function set.combineRange(this, combineRangeValue)
            valid = isscalar(combineRangeValue) && (isnumeric(combineRangeValue) || islogical(combineRangeValue));
            if ~valid
                matRad_cfg = MatRad_Config.instance();
                matRad_cfg.dispError('Invalid value for combineRange! Needs to be a boolean / logical value!');
            end
            this.combineRange = combineRangeValue;
            this.updateScenarios();
        end

        function set.combinations(this, combinationsValue)
            valid = any(strcmp(combinationsValue, this.validCombinationTypes));
            if ~valid
                matRad_cfg = MatRad_Config.instance();
                matRad_cfg.dispError(['Invalid value for combinations! Needs to be one of the strings %s!'], ...
                                     strjoin(this.validCombinationTypes, ' / '));
            end
            this.combinations = combinationsValue;
            this.updateScenarios();
        end
    ...
    methods (Access = protected)
        ...
        function validateScenarioDimensionSupport(~, scenarioDimensionActive)
            if any(strcmp(scenarioDimensionActive, 'gantry')) || any(strcmp(scenarioDimensionActive, 'couch'))
                matRad_cfg = MatRad_Config.instance();
                matRad_cfg.dispError(['Gantry/couch uncertainty dimensions are only supported by random and nominal ' ...
                                      'scenario models for now. Use rndScen for sampled angular uncertainty.']);
            end
        end
