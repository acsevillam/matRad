...
%
% output
%   indices:        indices of voxels of the ct cube that are inside the
%                   contour
%
% References
%   -
...
for i = 1:size(structure.item,2)
    ...
    if ~isempty(structure.item(i).points)
        ...
        contourSlicePos = unique(structure.item(i).points(:,3));

        if numel(contourSlicePos) > 1 || isempty(contourSlicePos)
            matRad_cfg.dispError('Contour defined over multiple planes!');
        end

        [dicomCtSlicePos,dicomCtSliceThickness] = matchDicomCtSlice(ct.dicomInfo.SlicePositions, ...
            ct.dicomInfo.SliceThickness,contourSlicePos);

        %Sanity check
        msg = checkSliceThickness(dicomCtSliceThickness);
        if ~isempty(msg)
            matRad_cfg.dispError('Slice Thickness of slice at %f could not be identified: %s',contourSlicePos,msg);
        end
...
function [slicePosition,sliceThickness] = matchDicomCtSlice(slicePositions,sliceThicknesses,contourSlicePos)
slicePosition = [];
sliceThickness = [];

if isempty(slicePositions) || isempty(sliceThicknesses)
    return;
end

slicePositions = slicePositions(:);
sliceThicknesses = sliceThicknesses(:);
[sliceDistance,sliceIx] = min(abs(slicePositions - contourSlicePos));

if isscalar(sliceThicknesses)
    candidateThickness = sliceThicknesses;
elseif numel(sliceThicknesses) == numel(slicePositions)
    candidateThickness = sliceThicknesses(sliceIx);
else
    return;
end

if sliceDistance <= max(candidateThickness / 2,1e-3)
    slicePosition = slicePositions(sliceIx);
    sliceThickness = candidateThickness;
end
end
