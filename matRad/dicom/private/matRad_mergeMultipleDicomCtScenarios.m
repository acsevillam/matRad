...
ct.x = getCommonCtField(tmpCt,'x',matRad_cfg);
ct.y = getCommonCtField(tmpCt,'y',matRad_cfg);
ct.z = getCommonCtField(tmpCt,'z',matRad_cfg);
ct.cubeDim = getCommonCtField(tmpCt,'cubeDim',matRad_cfg);
...
function value = getCommonCtField(ctScenarios,fieldName,matRad_cfg)
value = ctScenarios{1}.(fieldName);

for ctPhase = 2:numel(ctScenarios)
    if ~isequal(ctScenarios{ctPhase}.(fieldName),value)
        matRad_cfg.dispError('There are scenarios with incompatible %s values.',fieldName);
    end
end
end
