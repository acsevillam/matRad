...
switch calcType
    case 'relative'
        priceIndex.mean=(qiRob.mean-qiNom.mean)/qiNom.mean;
        
        for runDX = 1:numel(refVol)
            if(qiNom.(strcat('D_',num2str(refVol(runDX))))>0)
                priceIndex.(strcat('D_',num2str(refVol(runDX)))) = (qiRob.(strcat('D_',num2str(refVol(runDX))))-qiNom.(strcat('D_',num2str(refVol(runDX)))))/qiNom.(strcat('D_',num2str(refVol(runDX))));
            end
        end
        ...
        for runVX = 1:numel(refGy)
            ...
            if(qiNom.(['V_' strrep(sRefGy,'.','_') 'Gy'])>0)
                priceIndex.(['V_' strrep(sRefGy,'.','_') 'Gy']) = (qiRob.(['V_' strrep(sRefGy,'.','_') 'Gy'])-qiNom.(['V_' strrep(sRefGy,'.','_') 'Gy']))/qiNom.(['V_' strrep(sRefGy,'.','_') 'Gy']);
            end
        end
    case 'absolute'
